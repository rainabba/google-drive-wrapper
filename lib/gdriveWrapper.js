    ...
    this.drive.files.create({
        ...
        (err, response) => {
          if ( !err && response.status == "200" ) {
            complete( null, response.data )
          } else {
            complete( err || "Error uploading file from gdriveWrapper.uploadFile()", response.data )
          }
        });
}
            ...
            setImmediate(function() {
                if (!errorOccurred) {
                    if (requestOutput.response.data.statusCode != 200) {
                        complete(new Error(requestOutput.response.data.statusMessage), null, destFilename);
                    } else {
                        complete(null, meta);
                    }
                }
        ...
        wrapperThis.drive.files.list({
            ...
            function(err, response) {
                ...
                if (index < (fileComponents.length - 1)) {
                    ...
                    complete(null, response.data.files[0]);
                }
            }
        );
    }

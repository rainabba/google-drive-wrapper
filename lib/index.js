...
let drive,
    ncfs = { 
        Drive: {
            pathCache: new NodeCache( { stdTTL: 60*60*24, checkperiod: 60*60 } ),
        },
    }
...
ncfs.Drive.getMetaForFilename = function( drivePath ) {
    ...
    return new Promise((resolve, reject) => {
        ...
        function findNext (parent, fileComponents, index) {
            ...
            function handleResponse( response, setCache = true ) {
                if ( setCache && response.data.files[0] ) {
                    cacheSet( query, { data: response.data } ); // Re-wrap to ensure we're only getting data
                }

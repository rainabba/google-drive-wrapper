...
const fs = require('fs'),
    ...
    async = require('async'),
        ...
        throttle = require('promise-ratelimit');
...
let drive,
    ncfs = {
        Drive: {
            pathCache: null,
            apiThrottle: null, // setup in constructor to make rate user controllable
        },
    };
            ...
            if (response) {
                ...
                ncfs.Drive.apiThrottle().then(() => {
                    self.drive.files.list({
                            spaces: 'drive',
                            pageSize: ncfs.Drive.options.pageSize,
                            q: query,
                        })
                        .then(response => {
...
module.exports = {
    Drive: function(auth, google, options) {
        ...
        if (typeof auth == 'object' && typeof google == 'object') {
            ...
            ncfs.Drive.options = Object.assign(defaultOptions, options);
            ncfs.Drive.apiThrottle = throttle(ncfs.Drive.options.throttle); // miliseconds
            ncfs.Drive.pathCache = new NodeCache({ stdTTL: ncfs.Drive.options.cacheTTL, checkperiod: ncfs.Drive.options.cacheCheckperiod })
            return ncfs.Drive;
        } else {
